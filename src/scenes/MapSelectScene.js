...
// Pick the battlefield after choosing a champion. The map drives the theme and
// has feature modifiers (cover / hazards / loot).
export default class MapSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    const champ = getCharacter(this.characterId);

    this.add.text(width / 2, 40, 'CHOOSE YOUR BATTLEFIELD', {
      fontFamily: 'monospace', fontSize: '36px', color: '#ffd700', fontStyle: 'bold',
    }).setOrigin(0.5);
    this.add.text(width / 2, 80, `Leading ${champ.name}`, {
      fontFamily: 'monospace', fontSize: '15px', color: '#c9c4e0',
    }).setOrigin(0.5);
    ...
    const pages = Math.ceil(MAPS.length / perPage);
    this._page = 0;
    this._pageObjs = [];
    ...
    const renderPage = () => {
      ...
      const slice = MAPS.slice(start, start + perPage);
      const totalW = slice.length * cardW + (slice.length - 1) * gap;
      const startX = (width - totalW) / 2 + cardW / 2;
      slice.forEach((m, j) => {
        const objs = this.buildCard(startX + j * (cardW + gap), cy, cardW, m, start + j);
    ...
    this.add.text(width / 2, height - 26, `Press 1–${MAPS.length} or click a battlefield`, {
      fontFamily: 'monospace', fontSize: '13px', color: '#7d7896',
    }).setOrigin(0.5);

    this.input.keyboard.on('keydown', (e) => {
      if (e.key === 'Escape') { this.scene.start('MenuScene'); return; }
      const n = parseInt(e.key, 10);
      if (n >= 1 && n <= MAPS.length) this.start(MAPS[n - 1]);
    });
  ...
  buildCard(cx, cy, w, m, i) {
    ...
    reg(this.add.text(cx, top + 18, `${i + 1}. ${m.name}`, {
      fontFamily: 'monospace', fontSize: '16px', color: '#ffffff', fontStyle: 'bold',
    })).setOrigin(0.5);
    reg(this.add.text(cx, top + 134, m.civ, {
      fontFamily: 'monospace', fontSize: '12px', color: theme.accentCss,
    })).setOrigin(0.5);
    reg(this.add.text(cx, top + 160, m.blurb, {
      fontFamily: 'monospace', fontSize: '11px', color: '#c9c4e0',
      align: 'center', wordWrap: { width: w - 24 },
    })).setOrigin(0.5, 0);

    const feat = [];
    if (m.mods.obstacles >= 1.3) feat.push('+Cover');
    else if (m.mods.obstacles <= 0.7) feat.push('Open');
    if (m.mods.hazards >= 1.3) feat.push('+Hazards');
    else if (m.mods.hazards <= 0.7) feat.push('Few traps');
    if (m.mods.breakables >= 1.3) feat.push('+Crates');
    reg(this.add.text(cx, top + h - 30, feat.join('  •  ') || 'Balanced', {
    ...
    zone.on('pointerdown', () => this.start(m));
  ...
  start(map) {
    Audio.resume();
    Audio.startMusic();
    this.scene.start('GameScene', { characterId: this.characterId, mapId: map.id });
  }
}
